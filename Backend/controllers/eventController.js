...
const showEvents = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Error when fetching the events!", error: err });
  }
};
...
const addEvent = async (req, res) => {
  try {
    ...
    return res
      .status(201)
      .json({ message: "Event added successfully!", data: event });
  } catch (err) {
...
const editEvent = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "successfully edited the event!", data: result });
  } catch (err) {
